...
###############################
# Weighted least squares model by hand.
###############################
# convert vector w into matrix W etc
w = 1 / (sds**2)
W = np.diag(w)
ones = np.ones(len(y))
Xtr = [ones, x]
...
# find weighted least squares solution by hand
temp = Xtr * W * X
tempinv = temp.inv()
...
##############################################
# WLS compare to standard WLS library output.
##############################################
mod_wls = sm.WLS(y, X, weights=w )
res_wls = mod_wls.fit()
print('\n\nWLS LIBRARY MODEL SUMMARY') 
print(res_wls.summary())

##############################################
# OLS Estimate OLS model for comparison:
##############################################
res_ols = sm.OLS(y, X).fit()
print('\n\nOLS LIBRARY MODEL SUMMARY') 
print(res_ols.params)
print(res_wls.params)
print(res_ols.summary())

##############################################
# PLOT OLS and Weighted LS best fitting lines.
##############################################
fig = plt.figure(1)
fig.clear()
...
# OLS
plt.plot(x, res_ols.fittedvalues, "r--",label="OLS")
